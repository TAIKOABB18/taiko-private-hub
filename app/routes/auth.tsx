      ...
      <div className="relative mx-auto flex min-h-svh max-w-7xl flex-col px-6 py-6 lg:px-12">
        ...
        <div className="grid flex-1 items-center gap-12 py-12 lg:grid-cols-[1.1fr_.8fr]">
          <section>
            ...
            <h1 className="max-w-2xl text-5xl font-bold leading-[1.02] sm:text-7xl">
              Tu espacio privado.<br />Tus proyectos.<br />
              <span className="bg-gradient-to-r from-cyan-300 to-violet-400 bg-clip-text text-transparent">Sin límites.</span>
            </h1>
            <p className="mt-7 max-w-lg text-lg leading-8 text-slate-300">
              Archivos, chat, proyectos y mucho más.<br />Todo en un entorno seguro, privado y bajo tu control.
            </p>
          ...
          <Card className="border-slate-700 bg-slate-900/80 p-2 shadow-2xl shadow-blue-950/40 backdrop-blur">
            <CardHeader className="pb-2 text-center">
              ...
              <p className="text-sm text-slate-400">Accede a tu espacio privado</p>
            </CardHeader>
            <CardContent className="grid gap-4">
              ...
              <form onSubmit={submit} className="grid gap-4">
                {mode === "register" && (
                  <Input value={name} onChange={(event) => setName(event.target.value)} required autoComplete="name" placeholder="Tu nombre" className="border-slate-700 bg-slate-950/70 text-white" />
                )}
                <Input
                  value={mode === "register" ? OWNER_EMAIL : email}
                ...
                <div className="relative">
                  ...
                  />
                  <button type="button" aria-label="Mostrar contraseña" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-2.5 text-slate-400">
                    {showPassword ? <EyeOff className="size-4" /> : <Eye className="size-4" />}
                  </button>
                </div>
                <Button disabled={loading} className="h-11 bg-blue-600 hover:bg-blue-500 disabled:opacity-60">
                  {loading ? "Comprobando…" : mode === "register" ? "Crear cuenta de administrador" : "Entrar"}
              ...
              <Button type="button" onClick={() => setNotice("Abre el enlace privado de invitación que recibiste. Ese enlace contiene el acceso al proyecto autorizado.")} variant="outline" className="border-slate-700 bg-transparent text-white">
                <Link2 className="size-4" /> Entrar con invitación
              </Button>
